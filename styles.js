...
function onMonthly() {
    lastTime ="Month"
    work =[103,23,13,11,21,7]
    previousWork = [128,29,19,18,23,11]
    addi()
...
let style = styles.shift()
    let Svg = mainSvg.shift()
    let dailyHrs = work.shift()
    let previousHrs = previousWork.shift()
    if (dailyHrs>1 )
    {
        dailyHrs+=hrs
    }
    ...
    if (previousHrs>1) {previousHrs+=hrs }
    else{previousHrs+=hr}
set +=  `<div style="background-color:${style} ;" class="main_about">
${Svg}
              <div class="con">
          <div class="title">
            <p>${title}</p>
          ...
          <div class="time">
            <div id="#${title} " class="rtime">${dailyHrs}</div>
            <div class="ltime">
              <p>Last ${lastTime}-<span>${previousHrs}</span> </p>
            </div>
          </div>
              </div>
              </div>`
}
    document.getElementById("addi").innerHTML = set
}
